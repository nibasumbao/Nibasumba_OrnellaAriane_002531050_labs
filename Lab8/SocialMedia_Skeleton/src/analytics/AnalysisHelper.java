...
public class AnalysisHelper {
    ...
    for (Comment c : data.getComments().values()) {
        ...
        if (commentWithMaxLikes == null || 
            c.getLikes() > commentWithMaxLikes.getLikes()) {
    ...
    System.out.println("Q2 - Post with most liked comments: " + post);
}
    ...
    public void getPassiveAndActiveOverallUsers(){
        ...
        for(int i = 0; i< 5; i ++){
            System.out.println(users.get(i)+ "- Coverall Count:" + overallNumbers.get(users.get(i).getId()));
        }
